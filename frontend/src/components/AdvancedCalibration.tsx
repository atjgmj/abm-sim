...
export const AdvancedCalibration: React.FC<Props> = ({ scenario, onCalibratedScenario }) => {
  ...
  const [mlOptimizationResult, setMLOptimizationResult] = useState<any>(null);
  ...
  const [trainingDataStatus, setTrainingDataStatus] = useState<any>(null);
  ...
  const handleMLOptimization = async () => {
    setIsLoading(true);
    try {
  ...
  const applyMLOptimization = () => {
    if (mlOptimizationResult?.optimized_scenario) {
      onCalibratedScenario(mlOptimizationResult.optimized_scenario);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="flex space-x-8">
          {[
            ...
            { id: 'mlopt', label: 'ML最適化', icon: Calculator },
      ...
      {activeTab === 'mlopt' && (
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-lg border">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-lg font-medium text-gray-900">機械学習パラメータ最適化</h4>
              <Tooltip content="過去のシミュレーションデータを基に機械学習でパラメータを最適化" position="left" />
            </div>
            ...
            {trainingDataStatus && (
              <div className="mb-6">
                <div className="bg-gray-50 p-4 rounded-lg">
                  <h5 className="font-medium text-gray-900 mb-3">トレーニングデータ状況</h5>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">
                        {trainingDataStatus.total_simulations}
                      </div>
                      <div className="text-sm text-gray-600">シミュレーション数</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">
                        {trainingDataStatus.recommended_minimum}
                      </div>
                      <div className="text-sm text-gray-600">推奨最小数</div>
                    </div>
                    <div className="text-center">
                    ...
                    <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          disabled={isLoading}
                          className="flex items-center px-4 py-2 bg-yellow-600 text-white rounded-md hover:bg-yellow-700 disabled:opacity-50 text-sm font-medium"
                        >
            ...
            <div className="mb-6">
              <h5 className="font-medium text-gray-900 mb-3">最適化目標</h5>
              <div className="space-y-3">
                {optimizationTargets.map((target, idx) => (
                  <div key={idx} className="grid grid-cols-1 md:grid-cols-4 gap-3 p-3 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <select
                        value={target.kpi}
                        onChange={(e) => {
                          const newTargets = [...optimizationTargets];
                          newTargets[idx].kpi = e.target.value;
                          setOptimizationTargets(newTargets);
                        }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                      >
                        <option value="awareness">認知度</option>
                        <option value="interest">関心</option>
                        <option value="intent">意向</option>
                        <option value="knowledge">知識</option>
                        <option value="liking">好意</option>
                      </select>
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const newTargets = [...optimizationTargets];
                          newTargets[idx].target_value = parseInt(e.target.value);
                          setOptimizationTargets(newTargets);
                        }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const newTargets = [...optimizationTargets];
                          newTargets[idx].weight = parseFloat(e.target.value);
                          setOptimizationTargets(newTargets);
                        }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                      />
                    ...
                    <div className="flex items-end">
                      <button
                        onClick={() => {
                          const newTargets = optimizationTargets.filter((_, i) => i !== idx);
                          setOptimizationTargets(newTargets);
                        }}
                ...
                <button
                  onClick={() => {
                    setOptimizationTargets([
                      ...optimizationTargets,
                      { kpi: 'awareness', target_value: 1000, weight: 0.5 }
                    ]);
                  }}
            ...
            <div className="flex justify-between items-center mb-6">
              <p className="text-sm text-gray-600">
                機械学習アルゴリズムを使用してパラメータを最適化します
              </p>
              
              <button
                onClick={handleMLOptimization}
                disabled={isLoading}
                className="flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-md hover:from-purple-700 hover:to-blue-700 disabled:opacity-50 font-medium"
              >
                {isLoading ? <RefreshCw className="h-5 w-5 mr-2 animate-spin" /> : <Calculator className="h-5 w-5 mr-2" />}
                ML最適化実行
              </button>
            </div>
            ...
            {mlOptimizationResult && (
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-6 rounded-lg">
                  <h5 className="font-medium text-gray-900 mb-4">最適化結果</h5>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-purple-600">
                        {mlOptimizationResult.optimization_result.improvement.toFixed(1)}%
                      </div>
                      <div className="text-sm text-gray-600">改善予測</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">
                        {(mlOptimizationResult.optimization_result.confidence * 100).toFixed(0)}%
                      </div>
                      <div className="text-sm text-gray-600">信頼度</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-medium text-gray-800">
                        {mlOptimizationResult.optimization_result.method_used}
                      </div>
                      <div className="text-sm text-gray-600">手法</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-medium text-gray-800">
                        {mlOptimizationResult.optimization_result.training_samples}
                      </div>
                      <div className="text-sm text-gray-600">学習サンプル数</div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <ul className="space-y-2">
                      {mlOptimizationResult.recommendations.map((rec: string, idx: number) => (
                        <li key={idx} className="text-sm text-blue-800 flex items-start">
                          <Lightbulb className="h-4 w-4 text-blue-600 mr-2 flex-shrink-0 mt-0.5" />
                          {rec}
                        </li>
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-md hover:from-purple-700 hover:to-blue-700 font-medium"
                  >
                    <CheckCircle className="h-5 w-5 mr-2" />
                    最適化パラメータを適用
                  </button>
